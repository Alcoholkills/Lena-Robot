...
PATH = r"/Users/pierre/chromedriver"
...
#        file = json.load(jfile)["0"]
    ...
    for x in file:
        email = x["options"]["login"]
        break
    requests.post("https://"+ urlForDB + "/api/session/new",headers={"accept":"application/ld+json","Content-Type": "application/ld+json"}, json={"id":thisSession, "email":email})
    robot(file,thisSession)
